...
/**
 * Détail des actions "façon sweeper tracer" (asin/tote/station/occurrences)
 * pour un login, sur une fenêtre de temps [startISO, endISO[. Regroupe les
 * trace_events de TOUTES les sessions 'trace' dont la plage (range_start/
 * range_end) recoupe la fenêtre demandée — pas juste la toute dernière
 * analyse — pour que ça marche aussi bien avec l'onglet "Aujourd'hui" que
 * "Globale" (aucune borne) ou "Date" (un jour précis dans le passé).
 * Si startISO/endISO sont null, aucune borne n'est appliquée (onglet "Globale").
 */
function getTraceEventsForWindow(login, startISO, endISO) {
  let query = `
    SELECT te.* FROM trace_events te
    JOIN sessions s ON te.session_id = s.id
    WHERE s.login = ? AND s.source = 'trace' AND s.error IS NULL
  `;
  const params = [login];
  if (startISO) { query += ` AND (s.range_end IS NULL OR s.range_end >= ?)`; params.push(startISO); }
  if (endISO) { query += ` AND (s.range_start IS NULL OR s.range_start < ?)`; params.push(endISO); }
  query += ` ORDER BY te.first_date ASC`;
