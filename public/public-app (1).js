...
// Pause programmée exclue des arrêts par le script FCR Lite Ultra (10h-10h30
// heure locale, tous les jours) — reprise ici uniquement pour l'affichage
// visuel sur la frise (elle n'est jamais comptée comme un "arrêt").
const FCR_SCHEDULED_BREAKS = [{ start: '10:00', end: '10:30' }];

// Frise horizontale : barre pleine = période active, blocs rouges = arrêts
// >15min (tooltip heure exacte + durée), blocs ambre = pause programmée.
// Garde-fou perf : au-delà de 2 jours de plage, la frise devient illisible
// (des centaines de traits d'heure) -> on affiche un message à la place.
function timelineSVG(rangeStartISO, rangeEndISO, idleGaps, { w = 220, h = 26, activeColor = '#29ffb0', idleColor = '#ff4d6a', breakColor = '#ffb300', showBreaks = true } = {}) {
  ...
  if (span > 2 * 86400000) {
    return `<div class="empty-state">Frise non affichée au-delà de 2 jours (période trop large pour rester lisible) — voir le tableau des arrêts ci-dessous.</div>`;
  }
  ...
  // Bandes de pause programmée : une par jour civil recoupant [start, end].
  let breakRects = '';
  if (showBreaks) {
    ...
    while (cursor.getTime() <= end) {
      for (const b of FCR_SCHEDULED_BREAKS) {
        const [sh, sm] = b.start.split(':').map(Number);
        const [eh, em] = b.end.split(':').map(Number);
        const bs = new Date(cursor); bs.setHours(sh, sm, 0, 0);
        const be = new Date(cursor); be.setHours(eh, em, 0, 0);
        const gs = Math.max(bs.getTime(), start), ge = Math.min(be.getTime(), end);
        if (ge > gs) {
          const x = clamp(((gs - start) / span) * w);
          const x2 = clamp(((ge - start) / span) * w);
          if (x2 > x) breakSegs.push({ x, width: Math.max(x2 - x, 1.5), from: bs.toISOString(), to: be.toISOString() });
        }
      }
      cursor = new Date(cursor.getTime() + dayMs);
    }
    breakRects = breakSegs.map(s => `<rect x="${s.x.toFixed(1)}" y="2" width="${s.width.toFixed(1)}" height="${h - 4}" rx="2" fill="${breakColor}" fill-opacity="0.55"><title>Pause programmée ${fmtDT(s.from)} → ${fmtDT(s.to)}</title></rect>`).join('');
  }
  ...
  <div class="timeline-legend">
    <span><span class="timeline-legend-dot" style="background:${idleColor}"></span>Arrêt &gt;15min</span>
    <span><span class="timeline-legend-dot" style="background:${breakColor}"></span>Pause programmée</span>
  </div>`;
}
...
// Ligne 3 : frise (arrêts + pause programmée en surbrillance).
function renderDetailTimelineRow(t) {
  const domainEnd = new Date(Math.min(new Date(t.dayEnd).getTime(), Date.now())).toISOString();
  const timeline = timelineSVG(t.dayStart, domainEnd, (t.idleOver15 || {}).gaps || [], { w: 900, h: 34 });
  return `<div class="chart-box" style="margin-top:14px;"><h3>Frise — arrêts &amp; pause en surbrillance</h3><div class="timeline-wrap">${timeline}</div></div>`;
}
// Ligne 4 : détail des arrêts.
function renderDetailIdleTable(t) {
  const gaps = ((t.idleOver15 || {}).gaps || []).slice().sort((a, b) => (a.from || '').localeCompare(b.from || ''));
  if (!gaps.length) {
    return `<div class="section-title">Arrêts &gt;15min détaillés</div><div class="empty-state">Aucun arrêt &gt;15min sur cette période 🎉</div>`;
  }
  const rows = gaps.map(g => `
    <tr><td>${fmtDT(g.from)}</td><td>${fmtDT(g.to)}</td><td class="num">${fmtDuration(g.durationMin)}</td></tr>`).join('');
  return `
    <div class="section-title">Arrêts &gt;15min détaillés</div>
    <table class="sessions-table"><thead><tr><th>De</th><th>À</th><th>Durée</th></tr></thead><tbody>${rows}</tbody></table>`;
}
// Ligne 5 : détail des actions, façon FCR Trace Sweeper (asin/tote/station/occurrences).
function renderTraceEventsTable(events, color) {
  if (!events.length) {
    return `<div class="section-title">Détail des actions (tracé)</div><div class="empty-state">Aucune action tracée sur cette période.</div>`;
  }
  ...
  return `
    <div class="section-title">Détail des actions (tracé)</div>
...
// Construit les 5 lignes pour un onglet donné. `traceWindow` = { start, end }
// en ISO (omis pour l'onglet "Globale" -> aucune borne appliquée côté serveur).
async function renderDetailBody(login, t, traceWindow) {
  const body = document.getElementById('detail-tabbody');
  const color = accentFor(login);
  body.innerHTML = renderDetailKpiRow(t)
    + renderDetailHourlyChart(t, color)
    + renderDetailTimelineRow(t)
    + renderDetailIdleTable(t)
    + `<div id="detail-trace-slot"><div class="section-title">Détail des actions (tracé)</div><div class="empty-state">Chargement…</div></div>`;
  try {
    const params = new URLSearchParams();
    if (traceWindow && traceWindow.start) params.set('start', traceWindow.start);
    if (traceWindow && traceWindow.end) params.set('end', traceWindow.end);
    const qs = params.toString();
    const events = await api(`/api/sweepers/${encodeURIComponent(login)}/trace-events${qs ? '?' + qs : ''}`);
    const slot = document.getElementById('detail-trace-slot');
    if (slot) slot.outerHTML = renderTraceEventsTable(events, color);
  } catch (e) {
    const slot = document.getElementById('detail-trace-slot');
    if (slot) slot.innerHTML = `<div class="section-title">Détail des actions (tracé)</div><div class="empty-state">Erreur : ${escapeHtml(e.message)}</div>`;
  }
}
...
async function loadDetailDate(login, dateStr) {
  ...
  try {
    ...
    const t = await api(`/api/sweepers/${encodeURIComponent(login)}/day?${params}`);
    await renderDetailBody(login, t, { start: start.toISOString(), end: end.toISOString() });
  } catch (e) {
    body.innerHTML = `<div class="empty-state">Erreur : ${escapeHtml(e.message)}</div>`;
  }
}
